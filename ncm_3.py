...
## TOTAL is not 1 or total is 1 cases
            if total == 1:
                ...
                for key in features:
                    #py2: if glob_scores.has_key(file):
...
##version of samtools
    ...
    for sample in bam_list:
        ...
        if version.startswith("0"):
            ...
            sys.exit("samtools not supported as samtools mpileup is depicated and replaced by bcftool mpileup.")
        else:
            #command = SAMTOOLS + " mpileup -uf " + REF + " -l " + bedFile + " " + sample + " | "  + BCFTOOLS + " call -c > " + outdir + "/" + tag  + ".vcf"
            command = BCFTOOLS + " mpileup -Oz -f " + REF + " -R " + bedFile + " " + sample + " | "  + BCFTOOLS + " call -Ov -c -o " + outdir + "/" + tag  + ".vcf"
        print (command)
